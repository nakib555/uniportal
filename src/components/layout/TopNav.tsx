    ...
    <header className="h-20 flex-shrink-0 flex items-center justify-between px-8 bg-white/50 dark:bg-stone-900/50 backdrop-blur-md border-b border-stone-200 dark:border-stone-800 z-50 sticky top-0 print:hidden relative">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {!store.isAdmin && (
            <button 
              onClick={() => portal.handleSmartRefresh()}
              disabled={portal.isSyncing}
            ...
            >
              ...
              <span className="hidden sm:inline-block">
                {portal.isSyncing ? 'Syncing...' : 'Smart Refresh'}
              </span>
            </button>
          )}
          ...
          <div className="flex items-center gap-3">
             <div className="flex items-center gap-3 rounded-full hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors pr-2 cursor-pointer">
               <ProxyImage src={profilePic || `https://api.dicebear.com/7.x/notionists/svg?seed=${student?.name || 'Student'}`} fallbackSrc={`https://api.dicebear.com/7.x/notionists/svg?seed=${student?.name || 'Student'}`} alt="Profile" className="w-9 h-9 rounded-full object-cover shadow-sm bg-stone-100 dark:bg-stone-800 border border-stone-200 dark:border-stone-700" />
      ...
      {/* Smart Refresh Success Pill */}
      <AnimatePresence>
        {portal.smartRefreshNotice && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-[82px] right-8 z-50 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 text-xs font-semibold px-4 py-2 rounded-full shadow-lg flex items-center gap-2 border border-stone-700/40 pointer-events-none"
          >
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 dark:text-emerald-600" />
            <span>{portal.smartRefreshNotice}</span>
          </motion.div>
        )}
      </AnimatePresence>
    </header>
